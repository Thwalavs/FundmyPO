...
export default function DashboardPage() {
  ...
  async function handleAcceptOffer(poId: string, offerId: string) {
    try {
      ...
      if (offerData && poData) {
        // Save to deal_approvals table — pending admin approval
        await supabase.from('deal_approvals').insert({
          po_id: poId,
          offer_id: offerId,
          funder_id: offerData.funder_id,
          supplier_id: user.id,
          amount: offerData.amount,
          interest_rate: offerData.interest_rate,
          term_days: offerData.term_days,
        ...
        // Notify admin of deal pending approval
        try {
        ...
        } catch (e) { console.log('Admin deal notification failed:', e) }

        // Notify supplier that offer accepted and pending admin approval
        try {
          await fetch('/api/send-email', {
            method: 'POST', headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              type: 'offer_accepted_supplier',
              to: supplierProfile?.email || user.email,
              data: {
              ...
              {pos.map(po => {
                ...
                return (
                  <div key={po.id} style={{ background: '#fff', border: '1px solid #e5e5e5', borderRadius: '12px', padding: '1.25rem' }}>
                    ...
                    {acceptedOffer && (
                      <div style={{ marginTop: '1rem', background: '#FEF3C7', border: '1px solid #F59E0B', borderRadius: '12px', padding: '1.25rem', textAlign: 'center' }}>
                        <p style={{ fontSize: '16px', fontWeight: '700', color: '#92400E', marginBottom: '.5rem' }}>Offer accepted — pending admin approval</p>
                        <p style={{ fontSize: '13px', color: '#92400E' }}>
                          Your accepted offer of R {acceptedOffer.amount.toLocaleString()} at {acceptedOffer.interest_rate}% is being reviewed by the FundMyPO team. You will be notified once approved.
                        </p>
                      </div>
                    )}
